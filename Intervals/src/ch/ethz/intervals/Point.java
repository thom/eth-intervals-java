...
public final class Point implements PointMirror {
	...
	private static AtomicIntegerFieldUpdater<Point> waitCountUpdater = AtomicIntegerFieldUpdater
			.newUpdater(Point.class, "waitCount");

	final String name;
	/** Possibly null */
	final Point bound;
	/** if a start point, the paired end point. otherwise, null. */
	final int flags;
	/** various flags */
	final int depth;
	/** depth of bound + 1 */

	private ChunkList<Point> outEdges;
	/** Linked list of outgoing edges from this point. */
	private volatile int waitCount;
	/** Number of preceding points that have not arrived. */
	private Interval interval;

	/**
	 * Interval which owns this point. Set to {@code null} when the point
	 * occurs.
	 */
	...
	/**
	 * Returns the interval with which this point is associated or null if this
	 * point has occurred. Note that returning non-null is not a guarantee that
	 * point has not occurred by the time you receive the return value!
	 */
	...
	@Override
	public final boolean isBoundedBy(PointMirror p) {
		if (p instanceof Point)
			return isBoundedBy((Point) p);
		return false; // always bounded by a genuine Point, not some funky
		// mirror
	}
	...
	/**
	 * Returns true if {@code this == p} or {@code this} <i>happens before</i>
	 * {@code p}
	 */
	@Override
	public boolean hbeq(final PointMirror p) {
		return (this == p) || hb(p);
	}

	/**
	 * Returns true if {@code this} <i>happens before</i> {@code p}, including
	 * speculative edges.
	 */
	 ...
	 *            another point
	 * @param skipFlags
	 *            Skips edges which have any of the flags in here.
	 */
	private boolean hb(final PointMirror tar, final int skipFlags) {
		...
		// Otherwise, since safety conditions are established
		// by the PRESENCE of paths and not their absence, we would
		// be able to remove synchronized checks if we could detect
		// cycles another way, or at least synchronize in some other way
		// when checking for cycles. Perhaps a global clock.
		...
		// If src has no outgoing edges, then it can only HB tar if its bound HB
		// tar:
		...
		class SearchHelper {
			...
			boolean tryEnqueue(Point pnt) {
				if (pnt != null) {
					// found tar
					// found start point of an ancestor of p
					if ((pnt == tar)
	...
	/**
	 * When a preceding point (or something else we were waiting for) occurs,
	 * this method is invoked. Once {@link #waitCount} reaches 0, invokes
	 * {@link Interval#didReachWaitCountZero(Point)}.
	 * 
	 * @param cnt
	 *            the number of preceding things that occurred, should always be
	...
	/**
	 * Invoked by {@link #arrive(int)} when wait count reaches zero, but also
	 * from {@link Intervals#inline(InlineTask)}
	 */
	...
	/**
	 * Invoked by {@link #interval} after wait count reaches zero. Each point
	 * occurs precisely once.
	 * 
	 * @param withError
	 *            true if there were uncaught exceptions that propagate to this
	 *            point.
	 */
	final void occur(boolean withError) {
		assert waitCount == 0;

		int newWaitCount = (withError ? OCCURRED_WITH_ERROR
				: OCCURRED_WITHOUT_ERROR);
	...
	/**
	 * Takes the appropriate action to notify a successor {@code pnt} that
	 * {@code this} has occurred. Propagates exceptions and optionally invokes
	 * {@link #arrive(int)}.
	 */
	private void notifySuccessor(Point pnt, boolean arrive) {
		if (didOccurWithError()) {
			/*
			 * No need to deliver the error to our bound: - If we originated the
			 * error, it's a vertical error delivered by us. - If someone else
			 * with same bound originated error, it's a vertical error delivered
			 * by them. - If someone else with a greater bound than us did so,
			 * it was done when the error was delivered to us. But we do need to
			 * deliver to anyone up to but not including the bound. See {@link
			 * TestErrorPropagation#predecessorsOfSubintervalEndThatDie()} for
			 * reason why.
			 */
			Point p = pnt;
			while (p != bound) {
	...
	/**
	 * Adds to the wait count. Only safe when the caller is one of the people we
	 * are waiting for. <b>Does not acquire a lock on this.</b>
	 */
	...
	/**
	 * Adds to the wait count but only if the point has not yet occurred. Always
	 * safe.
	 * 
	...
	/**
	 * Waits until this point has occurred. If this is a helper thread, tries to
	 * do useful work in the meantime!
	 */
	void join() {
	...
	/**
	 * Optimized routine for the case where 'this' is known to have already
	 * occurred and not to have had any exceptions.
	 */
	...
	/**
	 * Removes an edge to {@code toImpl}, returning true if this point has
	 * occurred.
	 */
	...
	void confirmEdgeAndAdjust(Point toImpl, int flags) {
		...
		synchronized (this) {
			if (!didOccur()) {
				toImpl.addWaitCount();
				ChunkList
						.removeSpeculativeFlagAndAdd(outEdges, toImpl, WAITING);
				return;
			} else {
