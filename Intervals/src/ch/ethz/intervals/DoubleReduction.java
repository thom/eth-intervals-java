...
public class DoubleReduction {
	...
	/**
	 * Technically, it's possible for a fork-join pool to grow more workers over
	 * time. Therefore, we actually keep a linked list of reductions. I would
	 * just grow the array, but I'd need a read-write lock then, and I don't
	 * want that.
	 */
	...
	/**
	 * Invoked from a worker interval to add some amount to the value that is
	 * returned on the next call to {@link #reduce()}.
	 * 
	 * @param amnt
	 *            amount to be added on next reduce
	 */
	 ...
	 * 
	 * @param amnt
	 *            amount to be subtracted on next reduce
	 */
	// @JPartMethod(effects="AtomicWr(@W)")
	...
	/**
	 * Computes total difference since last invocation, and resets accumulators.
	 */
	...
	/**
	 * Accumulates all changes from worker threads and returns the new value.
	 * 
	 * <p>
	 * A data-race occurs if this method overlaps with calls to
	 * {@link #add(double)} or {@link #subtract(double)}.
	 * 
	...
	/**
	 * Sets the current value to {@code v} and resets any interal counters.
	 * 
	 * <p>
	 * A data-race occurs if this method overlaps with calls to
	 * {@link #add(double)} or {@link #subtract(double)}.
	 ...
	 * <p>
	 * A data-race occurs if this method overlaps with calls to
	 * {@link #add(double)}, {@link #subtract(double)}, {@link #reduce()} etc.
	 */
	...
	/**
	 * Read current value without taking into account any calls to
	 * {@link #add(double)} or {@link #subtract(double)} that have occurred
	 * since the last {@link #reduce()} or {@link #resetAll(double)}.
	 * 
	 * <p>
	 * A data-race occurs if this method overlaps with calls to
	 * {@link #reduce()}, {@link #resetAll(double)}, or
	 ...
	 * <p>
	 * A data-race occurs if this method overlaps with calls to
	 * {@link #reduce()}, {@link #resetAll(double)}, or {@link #value()}.
	 */
