...
public class TestErrorPropagation {
	 ...
	 */
	@Test
	public void exceptionPropagatesToParent() {
		try {
			...
			Assert.assertTrue("Not subtype: " + e.getCause(),
					e.getCause() instanceof TestException);
		}
	}

	/**
	 * Tests that a catch method can return a new set to substitute a different
	 * exception.
	 */
	@Test
	public void catchMethodSubstitutesADifferentException() {
		try {
			Intervals.inline(new VoidInlineTask() {
				public void run(Interval subinterval) {
					new ThrowExceptionTask(subinterval) {
						...
						protected Set<? extends Throwable> catchErrors(
							...
							Assert.assertTrue("Not subtype: " + t,
									t instanceof TestException);
							return Collections
									.singleton(new UnsupportedOperationException());
						}
	 ...
	 */
	@Test
	public void catchMethodReturnsEmptySet() {
		Intervals.inline(new VoidInlineTask() {
			public void run(Interval subinterval) {
				new ThrowExceptionTask(subinterval) {
					@Override
	 ...
	 */
	@Test
	public void uncaughtExceptionsPreventSuccessorsFromExecuting() {
	 ...
	 */
	@Test
	public void addingChildrenAfterRunMethodFailsIsOk() {
		final AtomicInteger integer = new AtomicInteger();
		try {
			Intervals.inline(new VoidInlineTask() {
				public void run(Interval subinterval) {
	...
	private boolean containsSubtypeOf(Iterable<? extends Object> coll,
			Class<?> cls) {
		for (Object o : coll) {
			if (cls.isInstance(o))
				return true;
	 ...
	 */
	@Test
	public void addingChildrenDuringCatchErrorsIsUncool() {
		try {
			Intervals.inline(new VoidInlineTask() {
				public void run(Interval subinterval) {
					new ThrowExceptionTask(subinterval) {
			...
			Assert.assertEquals(2, e.allErrors().size()); // both the original
			// exc and new exc
			// are carried over
			Assert.assertTrue(containsSubtypeOf(e.allErrors(),
					TestException.class)); // original exc
			Assert.assertTrue(containsSubtypeOf(e.allErrors(),
					IntervalException.ParPhaseCompleted.class)); // new exc
		}
	}

	/**
	 * Check that when an error propagates from a pred of the end point, the
	 * interval is not cancelled but its successors are.
	 */
	@Test
	public void predecessorsOfEndThatDie() {
		final AtomicInteger integer = new AtomicInteger();
		try {
			Intervals.inline(new VoidInlineTask() {
				public void run(Interval subinterval) {
			...
			Assert.assertEquals(1, e.allErrors().size()); // both the original
			// exc and new exc
			// are carried over
			Assert.assertTrue(containsSubtypeOf(e.allErrors(),
					TestException.class)); // original exc
	 ...
	 */
	@Test
	public void predecessorsOfSubintervalEndThatDie() {
	...
	/**
	 * Check that when an interval has both internal errors and errors that
	 * propagate to its end point, both sets of errors reach the parent, and
	 * catchErrors() is called.
	 */
	@Test
	public void predecessorsOfEndThatDieAndInternalErrorsToo() {
		final AtomicInteger integer = new AtomicInteger();
		try {
			Intervals.inline(new VoidInlineTask() {
				public void run(Interval subinterval) {
			...
			// are carried over
			Assert.assertTrue(containsSubtypeOf(e.allErrors(),
					TestException.class)); // exc from thr
			Assert.assertTrue(containsSubtypeOf(e.allErrors(),
					UnsupportedOperationException.class)); // exc from a
	...
	/**
	 * Uncaught exceptions do not propagate if the MASK_EXC flag is set and an
	 * edge is later added.
	 */
	@Test
	public void maskExceptionsOnSpeculativeEdges() {
		...
		class TestHarness {
			...
			public void test() {
				try {
					...
					Assert.assertTrue("Not subtype: " + e.getCause(), e
							.getCause() instanceof TestException);
				}
			}
		}
		...
		// the end of h.savedInter has an associated exception, because
		// h.savedInter.end
	 ...
	 */
	@Test
	public void exceptionPropagatesToGrandparent() {
		class TestHarness {
			public void test() {
				try {
					Intervals.inline(new VoidInlineTask() {
	...
	@Test
	public void multipleExceptionsCollected() {
		class TestHarness {
			public void test(final int length) {
				try {
					...
					Assert.assertEquals("Wrong number of exceptions", length, e
							.allErrors().size());
				}
			}
		}
	...
	public void raceCycle1() {
		...
		inline(new VoidInlineTask() {
			public void run(final Interval subinterval) {
				...
				Assert.assertFalse("hb observed before final", b.end
						.hb(a.start));
				try {
				...
				Assert.assertTrue("hb not observed after final", b.end
						.hb(a.start));
			}
		});
			...
			public void run(final Interval subinterval) {
				new Interval(subinterval, "child") {
					...
					protected void run() {
						...
						// could undo the cycle. This could only happen in
						// practice
						// if another thread was simultaneously adding an edge,
						// we both found the cycle, and then they fixed theirs,
						// allowing
						// the scheduler to proceed, before we could fix ours.
						b.end.join();

						// Note: manually invoking this method doesn't cause a
						// CycleException
						Intervals.recoverFromCycle(b.end, a.start);
						Assert.assertFalse(
								"hb observed after recoverFromCycle", b.end
										.hb(a.start));
					}
				};
			}
	 ...
	 */
	@Test
	public void exceptionInitializingInlineInterval() {
		...
		Intervals.inline(new VoidInlineTask() {
			...
			public void run(Interval subinterval) {
				try {
					...
					Assert.assertTrue("Not subtype: " + exc.getCause(), exc
							.getCause() instanceof TestException);
					sb.append("X");
				}
			}
